...
// Synthetic activity dispatched while connected to a live call room, so supervising
// a call (which needs no mouse/keyboard) never idle-expires the session mid-call.
// The interval must stay well under WARNING_LEAD_MS so a connected call keeps the
