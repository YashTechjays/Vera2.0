...
and both call-list DTOs — the per-form timeline (`api/v1/patient_forms.py`) and the
tenant-wide history (`api/v1/calls.py::list_call_history`).

The live-call rule also exists in SQL form for list enumeration
...
def call_content_visible(
    initiated_by_id: UUID | None, published: bool, user_id: UUID | None, *, is_terminal: bool
) -> bool:
    ...
    until published."""
    return is_terminal or not call_hidden_from(initiated_by_id, published, user_id)
...
def recording_playable(
    *,
    ...
    place, not three."""
    visible = call_content_visible(initiated_by_id, published, user_id, is_terminal=is_terminal)
