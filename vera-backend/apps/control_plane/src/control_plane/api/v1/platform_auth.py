...
(zero PHI). MFA is mandatory: login always returns a `verify` challenge, completed at
`/mfa/verify`, which mints the `account_type='platform'`, `tenant_id=None` session. Failures
...
async def platform_login(
    ...
    # MFA is mandatory for platform operators (the seeded provider's enforce_mfa is always
    # True): login NEVER mints a session directly — it always hands back an MFA challenge.
