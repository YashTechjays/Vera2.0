...
async def prompts_world(
    ...
    async with sm() as s, s.begin():
        fs = FormSchema(
            insurance_type=InsuranceType.INFERTILITY_TREATMENT.value, name="IBV"
        )
        s.add(fs)
        await s.flush()
        sv = SchemaVersion(
    ...
    async with sm() as s, s.begin():
        ...
        await s.execute(
            text("DELETE FROM prompt WHERE id = :id").bindparams(id=ids.prompt_id)
        )
        await s.execute(
    ...
    async with sm() as s, s.begin():
        ...
        for tbl in ("audit_log", "user_role", "role_permission", "role"):
            await s.execute(
                text(f"DELETE FROM {tbl} WHERE tenant_id = :t").bindparams(t=tenant_id)
            )
        await s.execute(
            text("DELETE FROM user_role WHERE app_user_id = :u").bindparams(u=super_id)
        )
        await s.execute(
