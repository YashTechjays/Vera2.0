...
def _create_headers(token: str) -> dict[str, str]:
    """Auth + a fresh Idempotency-Key (mutating ingress requires one)."""
...
async def test_create_audits_form_intake_to_the_user_without_phi_values(
    client: httpx.AsyncClient,
    rbac_world: RBACWorld,
    admin_sessionmaker: async_sessionmaker[AsyncSession],
    ibv_schema: tuple[UUID, UUID],
    ...
    async with admin_sessionmaker() as session:
        row = (
            await session.execute(
                text(
                    "SELECT actor_type, actor_user_id, detail::text FROM audit_log "
                    "WHERE resource_id = :rid AND event_type = 'form.intake'"
                ).bindparams(rid=form_id)
            )
        ).one()
    actor_type, actor_user_id, detail = row
    # A session user, not the sheet's service key.
    assert actor_type == "user"
    assert actor_user_id == rbac_world.admin_id
    assert str(published_version_id) in detail
    assert '"answer_count": 16' in detail
    assert "patient_information" in detail  # section KEY
    assert "Jane Doe" not in detail  # never a value
    assert "CH-10293" not in detail
...
async def test_create_requires_idempotency_key(
    ...
    resp = await client.post(
        "/api/v1/patient-forms:create",
        json={"schema_id": str(schema_id), "intake_payload": INTAKE_PAYLOAD},
        headers=_auth(rbac_world.admin_token),  # no Idempotency-Key
    )
    assert resp.status_code == 400, resp.text


async def test_create_replayed_idempotency_key_is_rejected(
    client: httpx.AsyncClient,
    rbac_world: RBACWorld,
    rls_sessionmaker: async_sessionmaker[AsyncSession],
    ibv_schema: tuple[UUID, UUID],
    ...
    """A retried submit must not land a second PHI form — patient_form has no
    natural key, so the in-flight lock is the only de-dup there is."""
    schema_id, _ = ibv_schema
    headers = _create_headers(rbac_world.admin_token)
    ...
    async with tenant_session(rls_sessionmaker, rbac_world.tenant_id) as session:
        count = (
            await session.execute(
                ...
                .where(PatientForm.tenant_id == rbac_world.tenant_id)
            )
        ).scalar_one()
    assert count == 1
...
async def test_create_stale_published_version_returns_409(
    ...
    """The client renders one version and submits the id it rendered: if a different
    version got published meanwhile, binding it silently would break the lineage."""
    schema_id, _ = ibv_schema
    resp = await client.post(
        "/api/v1/patient-forms:create",
        json={
            ...
            "published_version_id": str(uuid7()),  # not the currently-published one
        },
        headers=_create_headers(rbac_world.admin_token),
    )
    assert resp.status_code == 409, resp.text
    assert "newer version" in resp.json()["message"]


async def test_create_accepts_the_matching_published_version_id(
    client: httpx.AsyncClient,
    rbac_world: RBACWorld,
    ibv_schema: tuple[UUID, UUID],
    cleanup_forms: None,
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["data"]["schema_version_id"] == str(published_version_id)
...
async def test_create_requires_forms_write(
    ...
    resp = await client.post(
        "/api/v1/patient-forms:create",
        json={"schema_id": str(schema_id), "intake_payload": INTAKE_PAYLOAD},
        headers=_create_headers(rbac_world.norole_token),
    )
    assert resp.status_code == 403, resp.text
