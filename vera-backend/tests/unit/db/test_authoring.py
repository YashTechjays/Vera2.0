...
version-per-schema partial index — plus that the seed data files parse."""
...
def test_manifest_and_schema_files_parse() -> None:
    manifest = json.loads((FORM_SCHEMA_DIR / "manifest.json").read_text())
    ...
    for entry in manifest:
        ...
        doc = json.loads((FORM_SCHEMA_DIR / entry["file"]).read_text())
