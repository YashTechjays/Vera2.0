...
`user_identity` (MFA left unenrolled) + a grant of the global SUPER_ADMIN role. The
operator sets up 2FA in the browser on first /platform/login (the enrollment wall);
no terminal QR. From then on, operators add each other via the platform invite flow
(separate plan).
...
async def bootstrap(
    ...
    *,
    ...
    """Create platform operator #1, returning True; or False (no-op) if a platform
    operator already exists. MFA is left unenrolled — the operator completes the
    browser enrollment wall on first /platform/login."""
