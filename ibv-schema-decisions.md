...
| 10 | `group_integrity` — keep + make consistent | **KEEP (builder uses it), and made CONSISTENT.** Purpose = re-ask unit: if a unit is incomplete on the call, re-ask that unit alone. Unit differs by table: **service-unit** (Infertility, Male Partner, General) → flag on the service group; **code-unit** (Diagnostic Labs) → flag per CPT row. Fix: (1) Diagnostic — moved gi off the `diagnostic_testing` group onto each of the 8 `cpt_*` rows; (2) added gi to the 5 unmarked service groups (office_visits, asc_professional, asc_facility, semen_analysis, sperm_cryo). Result: 13 group-level (all service tables) + 8 row-level (diagnostic). Grids (deductible/out_of_pocket/lifetime_maximum) left untouched — no `covered` gate, different semantics → flagged for reviewer. `group_integrity?` declared on `IbvField`. 25 tests, tsc, eslint green. | ✅ done |
